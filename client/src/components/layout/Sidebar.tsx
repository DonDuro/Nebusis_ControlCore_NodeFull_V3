...
export default function Sidebar({ workflows, complianceScores, overallProgress, isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <nav className={cn(
        "fixed left-0 top-0 h-full bg-nebusis-surface border-r border-nebusis-border overflow-y-auto flex flex-col z-40 transition-transform duration-300 ease-in-out",
        "w-64 sm:w-72 lg:w-64",
        isOpen ? "translate-x-0" : "-translate-x-full",
        "lg:translate-x-0 lg:relative lg:z-10"
      )}>
        {/* Mobile close button */}
        <div className="lg:hidden flex justify-between items-center p-4 border-b border-nebusis-border">
          <h2 className="text-lg font-semibold text-nebusis-slate">{t('common.nav')}</h2>
          <button
            onClick={onClose}
            className="touch-target p-2 rounded-lg hover:bg-nebusis-surface"
            aria-label={t('common.close')}
          >
            <X className="h-5 w-5 text-nebusis-slate" />
          </button>
        </div>
        ...
        <div className="p-4 pt-4 lg:pt-8 space-y-4 flex-1">
          ...
          <div className="mb-6">
            <div className="bg-nebusis-blue px-3 py-2 rounded-lg mb-3">
              <h3 className="text-sm font-semibold text-white">{framework === 'coso' ? 'COSO Components' : framework === 'intosai' ? 'INTOSAI Standards' : 'Control Components'}</h3>
            </div>
            <TooltipProvider>
                      ...
                      <TooltipTrigger asChild>
                        <div 
                          className={cn(
                            ...
                            statusBg,
                            isActive ? "bg-nebusis-blue" : "hover:bg-nebusis-surface"
                          )}
                          onClick={() => {
                        ...
                        >
                          <div className="flex items-center">
                            <IconComponent className={cn("w-4 h-4 mr-3", 
                              isActive ? "text-white" : "text-nebusis-slate"
                            )} />
                            <span className={cn("text-sm font-medium", 
                              isActive ? "text-white" : "text-nebusis-slate hover:text-nebusis-blue"
                            )}>{component.name}</span>
                          </div>
          ...
          {/* Separator */}
          <div className="border-t border-nebusis-border my-4"></div>
                  ...
                  <TooltipTrigger asChild>
                    <Link 
                      ...
                      className={cn(
                        ...
                        location === item.href 
                          ? "text-white bg-nebusis-blue" 
                          : "text-nebusis-slate hover:bg-nebusis-surface hover:text-nebusis-blue"
                      )}
