...
export default function Header({ user, institution, onMobileMenuToggle, stats }: HeaderProps) {
  ...
  return (
    <header className="bg-nebusis-surface border-b border-nebusis-border shadow-sm fixed top-0 left-0 right-0 z-40">
          ...
          <div className="flex items-center space-x-4">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className="lg:hidden touch-target bg-transparent hover:bg-nebusis-surface p-2 rounded-lg"
                    aria-label="Toggle mobile menu"
                  >
                    {mobileMenuOpen ? (
                      <X className="h-6 w-6 text-nebusis-slate" />
                    ) : (
                      <Menu className="h-6 w-6 text-nebusis-slate" />
                    )}
                  </button>
                </TooltipTrigger>
                <TooltipContent>
            ...
            <div className="flex items-center">
              ...
              <svg 
                ...
                height="28" 
                viewBox="0 0 32 32" 
                className="text-blue-600 flex-shrink-0 mr-3"
                fill="currentColor"
              >
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center space-x-4">
                  <div>
                    <h1 className="text-base sm:text-lg md:text-xl font-bold text-nebusis-slate leading-none">
                  ...
                  <p className="text-xs sm:text-sm text-nebusis-slate hidden sm:block">
                    {t('dashboard.title')}
                  </p>
                </div>
              </div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="flex items-center space-x-3 ml-4 h-auto p-2 hover:bg-gray-100">
                  <div className="text-right">
                    <p className="text-sm font-medium text-nebusis-slate">
                      {(user as any)?.name?.split(' - ')[0] || user.firstName + ' ' + user.lastName}
                    </p>
                    <p className="text-xs text-nebusis-muted">{user.role}</p>
                  </div>
                  {(user as any)?.email === 'calvarado@nebusis.com' ? (
                    ...
                    <div className="w-10 h-10 bg-nebusis-blue rounded-full flex items-center justify-center">
                      <span className="text-white text-sm font-medium">
                        {((user as any)?.name || user.firstName || 'U').charAt(0).toUpperCase()}
                      </span>
                    </div>
                  )}
                </Button>
